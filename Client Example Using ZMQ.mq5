...
//|                                Copyright 2017-2026, Ji-Feng Tsai |
//|                                        https://github.com/jiowcl |
//+------------------------------------------------------------------+
#property copyright   "Copyright 2026, Ji-Feng Tsai"
...
bool DetectEnvironment()
  {
    ...
    if (ServerReal == true && AccountInfoInteger(ACCOUNT_TRADE_MODE)==ACCOUNT_TRADE_MODE_DEMO)
      {
    ...
    while (!IsStopped())
      {
        ...
        if (message != "" && AccountInfoDouble(ACCOUNT_EQUITY) > 0.00)
          {
            singallogin     = -1;
            singalorderdata = "";
...
int MakeOrderOpen(const string symbol, 
                  ...
                  const string comment)
  {
    ...
    switch (vtype)
      {
        case POSITION_TYPE_BUY:
          ticketid = trade.Buy(vlots, symbol, vprice, sl, tp, comment); //OrderSend(symbol, POSITION_TYPE_BUY, vlots, vprice, order_slippage, sl, tp, comment, 0, 0, clrYellow);
          break;
    
        case POSITION_TYPE_SELL:
          ticketid = trade.Sell(vlots, symbol, vprice, sl, tp, comment);
          break;
    
        case ORDER_TYPE_BUY_LIMIT:
          if (openprice > 0.00)
            ticketid = trade.BuyLimit(vlots, openprice, symbol, sl, tp, ORDER_TIME_DAY, 0, comment); //OrderSend(symbol, POSITION_TYPE_BUYLIMIT, vlots, openprice, order_slippage, sl, tp, comment, 0, 0, clrYellow);
          break;
    
        case ORDER_TYPE_BUY_STOP:
          if (openprice > 0.00)
            ticketid = trade.BuyStop(vlots, openprice, symbol, sl, tp, ORDER_TIME_DAY, 0, comment); //OrderSend(symbol, POSITION_TYPE_BUYSTOP, vlots, openprice, order_slippage, sl, tp, comment, 0, 0, clrYellow);
          break;
    
        case ORDER_TYPE_SELL_LIMIT:
          if (openprice > 0.00)
            ticketid = trade.SellLimit(vlots, openprice, symbol, sl, tp, ORDER_TIME_DAY, 0, comment);
          break;
    
        case ORDER_TYPE_SELL_STOP:
          if (openprice > 0.00)
            ticketid = trade.SellStop(vlots, openprice, symbol, sl, tp, ORDER_TIME_DAY, 0, comment);
          break;
      }
...
bool MakeOrderPartiallyClose(const int ticketid, 
                             ...
                             const double tp)
  {
    ...
    if(type == POSITION_TYPE_BUY || type == POSITION_TYPE_SELL)
    {
      result = trade.PositionClosePartial(ticketid, lots);
    }
...
bool MakeOrderModify(const int ticketid, 
                     ...
                     const double tp)
  {
    ...
    if (OrderSelect(ticketid) == true)
      {         
        result = trade.OrderModify(ticketid, openprice, sl, tp, ORDER_TIME_DAY, 0); 
      }
...
int FindOrderBySingalComment(const string symbol, 
                             const int signal_ticketid)
  {
    ...
    for (orderindex=0; orderindex<ordersize; orderindex++)
      {
        if (position.SelectByIndex(orderindex) == false)
          continue;
          
        string ordercomment = position.Comment();
...
int FindClosedOrderByHistoryToComment(const string symbol, 
                                      const int signal_ticketid)
  {
    ...
    for (orderindex=0; orderindex<ordersize; orderindex++)
      {
        if (HistoryOrderSelect(ticketid) == false)
          continue;
         
        string ordercomment = HistoryOrderGetString(ticketid, ORDER_COMMENT);
        
        if (ordercomment == "")
          continue;
        
        if (symbol != HistoryOrderGetString(ticketid, ORDER_SYMBOL))
          continue;
         
        if (signal_ticketid != HistoryOrderGetInteger(ticketid, ORDER_TICKET))
          continue;
